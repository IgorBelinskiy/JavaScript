...
function form() {
  ...
  form.addEventListener('submit', function (event) {
    ...
    request.open('POST', 'server.php'); // request.setRequestHeader('Content-type', 'application/x-www-urlencoded');

    request.setRequestHeader('Content-type', 'application/json; charset=utf-8');
    var formData = new FormData(form); //Получаем все, что ответил пользователь в форме

    var obj = {};
    formData.forEach(function (value, key) {
      //заполнение obj значениями с formData
      obj[key] = value;
    });
...
function timer() {
  ...
  function getTimeRemaning(endtime) {
    //вычисляет остаток времени и вычлениет значения
    var t = Date.parse(endtime) - Date.parse(new Date()),
        seconds = Math.floor(t / 1000 % 60),
        minutes = Math.floor(t / 1000 / 60 % 60),
        hours = Math.floor(t / (1000 * 60 * 60)); // houers = Math.floor((t/1000/60/60) % 24),
  ...
  function setClock(id, endtime) {
    ...
    function updateClock() {
      ...
      function addZero(num) {
        //если цыфра меньше 10и, добавить '0'
        if (num <= 9) {
          return '0' + num;
        } else {
          return num;
      ...
      seconds.textContent = addZero(t.seconds); // hours.textContent = t.hours;
